...
int main (int argc, char **argv) {
  ...
  if(rank == 1) {
	...
	//Bob sends the encrypted messages to Alice
	MPI_Send(message, 5, MPI_UNSIGNED, 0, 1, MPI_COMM_WORLD);
	MPI_Send(a, 5, MPI_UNSIGNED, 0, 2, MPI_COMM_WORLD);
  }
  ...
  if(rank == 0) {
	MPI_Status status;
  	MPI_Recv(message, 5, MPI_UNSIGNED, 1, 1, MPI_COMM_WORLD, &status);
	MPI_Recv(a, 5, MPI_UNSIGNED, 1, 2, MPI_COMM_WORLD, &status);
